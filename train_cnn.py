...
qrnn_parameter = filter_dotdict_class_propoperty(param, CNNClassifier)

train_iter, dev_iter, vocab_size, label_size, pretrained_embedding_weight = load_dataset(
    param.train_dataset_path, param.dev_dataset_path, param.batch_size, param.max_text_length, param.embedding_dim, pretrained_word_embedding_name = param.pretrained_word_embedding_name, pretrained_word_embedding_path = param.pretrained_word_embedding_path, use_gpu = param.use_gpu
    )

model = CNNClassifier(**qrnn_parameter, vocab_size=vocab_size,label_size=label_size, pretrained_embedding_weight=pretrained_embedding_weight)
...
learning_logger = LearningLogger(param.result_folder_path, param.saved_model_file_path, param.train_log_folder_path, param.dev_log_folder_path)
